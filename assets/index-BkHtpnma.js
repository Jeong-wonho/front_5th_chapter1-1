...
`,g={login:o=>{if(o==="testuser"){const e={username:o,email:"",bio:""};return d.setUser("user",e),!0}return!1},logout:()=>{d.initUser("user")}};class y{constructor(){u(this,"handleLinkClick",e=>{if(console.log("handleLinkClick 실행"),e.target.tagName==="A"){e.preventDefault();const t=e.target.getAttribute("href"),s=t.includes("#")?t.split("#")[1]:t;if(console.log("handleLinkClick",s),t==="/logout"||s==="/logout"){console.log("/logout 실행"),g.logout(),this.navigateTo("/login");return}this.navigateTo(t)}});u(this,"handleLogin",e=>{e.preventDefault();const t=document.getElementById("username").value,s=document.getElementById("userPw").value;g.login(t,s)?this.navigateTo("/profile"):alert(!t||!s?"이름 또는 비밀번호를 입력해주세요.":"이름 또는 비밀번호가 일치하지 않습니다.")});u(this,"handleUpdateProfile",e=>{e.preventDefault();const t=document.getElementById("username").value.trim(),s=document.getElementById("email").value.trim(),n=document.getElementById("bio").value.trim();if(t){const r={username:t,email:s,bio:n};d.setUser("user",r),window.location.reload()}});this.routes={},this.currentPath=""}addRoute(e,t){this.routes[e]=t}renderPage(e){const t=this.routes[e];return typeof t=="function"?t():R()}setupPageListeners(e){console.log("setupPageListeners path",e),this.currentPath=e;const t=d.getUser("user");if(e==="/login"){if(t){this.navigateTo("/");return}const s=document.getElementById("login-form");s&&(s.removeEventListener("submit",this.handleLogin),s.addEventListener("submit",this.handleLogin))}if(e==="/profile"&&!t){this.navigateTo("/login");return}else{const s=document.getElementById("profile-form");s&&(s.removeEventListener("submit",this.handleUpdateProfile),s.addEventListener("submit",this.handleUpdateProfile))}if(e!=="/login"){console.log("nav 이벤트 리스너 등록");const s=document.querySelector("nav");s&&(s.removeEventListener("click",this.handleLinkClick),s.addEventListener("click",this.handleLinkClick))}}navigateTo(){throw new Error("자식 클래스에서 해당 메서드 구현해야합니다.")}handleRoute(){throw new Error("자식 클래스에서 해당 메서드 구현해야합니다.")}init(){throw new Error("자식 클래스에서 구현합니다.")}}class I extends y{constructor(){super(),this.isGhPages=window.location.hostname==="localhost",this.repoName="/front_5th_chapter1-1",window.addEventListener("popstate",this.handlePopState.bind(this))}cleanPath(e){if(this.isGhPages&&e.startsWith(this.repoName)){const t=e.slice(this.repoName.length);return t.startsWith("/")?t:"/"+t}return e}handlePopState(){console.log("브라우저 popstate 이벤트 발생"),this.handleRoute(window.location.pathname)}navigateTo(e){console.log("브라우저 라우터: navigateTo",e),history.pushState(null,"",e),this.handleRoute("root",e)}handleRoute(e,t){console.log("브라우저 라우팅 이동",t);const s=this.cleanPath(t),n=document.getElementById(e);n&&(n.innerHTML=this.renderPage(s),this.setupPageListeners(s))}init(){this.addRoute("/",v),this.addRoute("/login",f),this.addRoute("/profile",x),this.handleRoute("root",window.location.pathname)}}class C extends y{constructor(){super(),window.addEventListener("hashchange",this.handleHashChange.bind(this))}getCurrentPath(){return console.log("해시 라우터: getCurrentPath",window.location.hash),window.location.hash.slice(1)||"/"}handleHashChange(){console.log("해시 변경 감지됨");const e=this.getCurrentPath();this.handleRoute("root",e)}navigateTo(e){console.log("해시 라우터: navigateTo",e),window.location.hash=e}handleRoute(e,t){console.log("해시 라우터: handleRoute",t);const s=document.getElementById(e);s&&(s.innerHTML=this.renderPage(t),this.setupPageListeners(t))}init(){if(this.addRoute("/",v),this.addRoute("/login",f),this.addRoute("/profile",x),!window.location.hash)window.location.hash="/";else{const e=this.getCurrentPath();console.log("해시 라우터: init",e),this.handleRoute("root",e)}}}function S(o="browser"){switch(o.toLowerCase()){case"hash":return new C;case"browser":return new I}}const U=S("browser");U.init();
