...
`,u={getUser(e){return JSON.parse(localStorage.getItem(e))},setUser(e,t){localStorage.setItem(e,JSON.stringify(t))},initUser(e){localStorage.removeItem(e)}},I=()=>{const e=u.getUser("user")||{username:"",email:"",bio:""};return`
  <div class="bg-gray-100 min-h-screen flex justify-center">
    <div class="max-w-md w-full">
      ${p()}
      ${b()}
  ...
  `},r="/front_5th_chapter1-1/",v={[r+"login"]:P,[r+"profile"]:I,[r]:$,[r+"error"]:x};console.log("routePath",{routePath:v});const E=e=>{const t=v[e];return typeof t=="function"?t():x()},L=()=>{console.log("logoutUser 실행"),u.initUser("user")},U=e=>{if(e.target.tagName==="A"){e.preventDefault();const t=e.target.getAttribute("href");if(console.log("href",t),t.endsWith("logout")||t.endsWith("/logout")){console.log("logout 실행"),L(),i(r+"login");return}i(t)}},S=e=>{e.preventDefault();const t=document.getElementById("username").value,l=document.getElementById("userPw").value;if(console.log("email",t),console.log("password",l),t==="testuser"){const s={username:t,email:"",bio:""};u.setUser("user",s),i(r+"profile")}else alert(!t||!l?"이름 또는 비밀번호를 입력해주세요.":"이름 또는 비밀번호가 일치하지 않습니다.")},B=e=>{e.preventDefault();const t=document.getElementById("username").value.trim(),l=document.getElementById("email").value.trim(),s=document.getElementById("bio").value.trim();if(t){const o={username:t,email:l,bio:s};u.setUser("user",o),location.reload()}},i=e=>{console.log("navigateTo",e),history.pushState(null,"",e);const t=document.getElementById("root");if(t.innerHTML=E(e),e===r+"login"){if(u.getUser("user")){i(r+"profile");return}const s=document.getElementById("login-form");s&&s.addEventListener("submit",S)}if(e===r+"profile"){const l=u.getUser("user");if(console.log("loginInfo",l),!l){i(r+"login");return}const s=document.getElementById("profile-form");s&&s.addEventListener("submit",B)}if(e!==r+"login"&&e!==r+"logout"){console.log("login or logout");const l=document.querySelector("nav");l&&l.addEventListener("click",s=>{s.target.tagName==="A"&&U(s)})}};document.addEventListener("DOMContentLoaded",()=>{console.log("DOMContentLoaded",location.pathname),i(location.pathname)});window.addEventListener("popstate",()=>{console.log("popstate 이벤트 처리"),i(location.pathname)});
