...
`,p=()=>(console.log("Header component 랜더링"),`<header class="bg-blue-600 text-white p-4 sticky top-0">
    ...
    </header>`),b=()=>{const t="/front_5th_chapter1-1/";console.log("BASE_URL",t);const o=window.location.pathname.includes("hash.html")?"hash":"browser",n=()=>{if(o==="hash")return console.log("해시 라우팅: getCurrentPath",window.location.hash),window.location.hash.slice(1)||"/";{const l=window.location.pathname;if(l.startsWith(t)){const a=l.substring(t.length)||"/";return a.startsWith("/")?a:`/${a}`}return l}};console.log("getCurrentPath",n());const s=localStorage.getItem("user"),r=n(),c=r==="/",P=r==="/profile",m=c?"text-blue-600 font-bold":"text-gray-600",F=P?"text-blue-600 font-bold":"text-gray-600",i=l=>{if(!l)return t;if(o==="hash")return`#${l}`;{if(l==="/")return t;const a=l.startsWith("/")?l.slice(1):l;return console.log("cleanPath",a),`${t.endsWith("/")?t.slice(0,-1):t}/${a}`}};console.log("getHref",i());let h;return s?h=`
